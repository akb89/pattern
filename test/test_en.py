...
class TestInflection(unittest.TestCase):
    ...
    def test_indefinite_article(self):
        ...
        self.assertEqual(en.inflect.article("heir", function=en.DEFINITE),
                         "the")
    ...
    def test_pluralize(self):
        ...
        self.assertEqual("octopodes",
                         en.inflect.pluralize("octopus", classical=True))
        self.assertEqual("octopuses",
                         en.inflect.pluralize("octopus", classical=False))
        ...
        for v, lexeme1 in en.inflect.verbs.infinitives.items():
            ...
            for j in range(len(lexeme2)):
                if lexeme1[j] == lexeme2[j] or \
                                        lexeme1[j] == "" and \
                                        lexeme1[j > 5 and 10 or 0] == lexeme2[
                                    j]:
                    i += 1
                n += 1
        self.assertTrue(float(i) / n > 0.90)
    ...
    def test_tenses(self):
        ...
        self.assertTrue(
            (en.inflect.PRESENT, 1, en.inflect.SINGULAR) in en.inflect.tenses(
                "am"))
...
class TestQuantification(unittest.TestCase):
    ...
    def test_quantify(self):
        ...
        for a, s in (
                ...
                ({"carrot": 4, "parrot": 2},
                 "several carrots and a pair of parrots")):
...
class TestParser(unittest.TestCase):
    ...
    def _test_morphological_rules(self, function=en.parser.morphology.apply):
        """ For each word in WordNet that is not in Brill's lexicon,
            test if the given tagger((word, "NN")) yields an improved (word,
            tag).
    ...
    def test_default_suffix_rules(self):
        ...
        # Given are the scores for detection of nouns, verbs, adjectives and
        # adverbs.
    ...
    def test_apply_context_rules(self):
        ...
        for a, b in (  # Rule:
                ...
                ([["", "VB"], ["countries", ""]],
                 [["", "JJ"], ["countries", ""]]),  # NEXTWD
                ([["close", "VB"], ["to", ""]], [["close", "RB"], ["to", ""]]),
                # RBIGRAM
                (
                [["very", ""], ["much", "JJ"]], [["very", ""], ["much", "RB"]]),
                # LBIGRAM
                (
    ...
    def test_find_chunks(self):
        ...
        self.assertEqual(v, [["black", "JJ", "B-NP", "O"],
                             ["cat", "NN", "I-NP", "O"]])
        # Assert the accuracy of the chunker.
        # For example, in "The very black cat must be really meowing really
        # loud in the yard.":
        # - "The very black" (NP)
        ...
        self.assertEqual(v,
                         [["", "RB", "B-ADVP", "O"], ["", "PRP", "B-NP", "O"]])
        # - "very nice cats" (NP)
        v = en.parser.find_chunks([["", "RB"], ["", "JJ"], ["", "PRP"]])
        self.assertEqual(v, [["", "RB", "B-NP", "O"], ["", "JJ", "I-NP", "O"],
                             ["", "PRP", "I-NP", "O"]])
    ...
    def test_find_prepositions(self):
        ...
        v = en.parse(
            "The cat was looking at me from up on the roof with interest.",
            prepositions=True)
        self.assertEqual(v,
                         "The/DT/B-NP/O cat/NN/I-NP/O " \
                         "was/VBD/B-VP/O looking/VBG/I-VP/O " \
                         "at/IN/B-PP/B-PNP me/PRP/B-NP/I-PNP " \
                         "from/IN/B-PP/B-PNP up/IN/I-PP/I-PNP "
                         "on/IN/I-PP/I-PNP the/DT/B-NP/I-PNP "
                         "roof/NN/I-NP/I-PNP " \
    ...
    def test_named_entity_recognition(self):
        ...
        self.assertEqual(v,
                         "Arnold/NNP-PERS Schwarzenegger/NNP-PERS is/VBZ "
                         "cool/JJ ./."
                         )
    ...
    def test_parse(self):
        ...
        self.assertTrue(
            isinstance(en.parse("ø ü", tokenize=True, tags=False, chunks=False),
                       unicode))
        self.assertTrue(isinstance(
            en.parse("ø ü", tokenize=False, tags=False, chunks=False), unicode))
        ...
        i, n = 0, 0
        for corpus, a in (("tagged-en-wsj.txt", (0.968, 0.945)),
                          ("tagged-en-oanc.txt", (0.929, 0.932))):
            for sentence in open(
                    os.path.join(PATH, "corpora", corpus)).readlines():
    ...
    def test_tagged_string(self):
        ...
        v = en.parser.parse("The black cat sat on the mat.", relations=True,
                            lemmata=True)
    ...
    def test_ngrams(self):
        ...
        self.assertEqual(v2, [("The", "cat", "is"), ("cat", "is", "napping"),
                              ("is", "napping", ".")])
        s = "The cat purrs. The dog barks."
        v1 = en.ngrams(s, n=2)
        v2 = en.ngrams(s, n=2, continuous=True)
        self.assertEqual(v1, [("The", "cat"), ("cat", "purrs"), ("The", "dog"),
                              ("dog", "barks")])
        self.assertEqual(v2,
                         [("The", "cat"), ("cat", "purrs"), ("purrs", "The"),
                          ("The", "dog"), ("dog", "barks")])
    ...
    def test_command_line(self):
        ...
        v = v.strip()
        self.assertEqual(v,
                         "Nice/JJ/B-NP/O/O/nice cat/NN/I-NP/O/O/cat "
                         "././O/O/O/.")
...
class TestParseTree(unittest.TestCase):
    ...
    def test_text(self):
        # Assert Text.
        self.assertEqual(self.text.sentences[0].string,
    ...
    def test_chunk_modifiers(self):
        ...
        self.assertEqual(v.chunk[2].modifiers,
                         [v.chunk[0]])  # should <=> perhaps
    ...
    def test_word_custom_tags(self):
        ...
        s = en.Sentence("onion/NN/FOOD",
                        token=[en.WORD, en.POS, "semantic_type"])
        v = s.words[0]
...
class TestModality(unittest.TestCase):
    ...
    def test_modality(self):
        ...
        # Given are the scores for the CoNLL-2010 Shared Task 1 Wikipedia
        # uncertainty data:
...
class TestSentiment(unittest.TestCase):
    ...
    def test_sentiment(self):
        ...
        self.assertTrue(
            en.sentiment(en.Text(en.parse("A bad book. Really horrible.")))[
                0] < 0)
        # Assert that :) and :( are recognized.
        self.assertTrue(en.sentiment(":)")[0] > 0)
        self.assertTrue(en.sentiment(":(")[0] < 0)
        # Assert the accuracy of the sentiment analysis (for the positive
        # class).
        ...
        # Assert the accuracy of the sentiment analysis on short text (for
        # the positive class).
    ...
    def test_sentiment_twitter(self):
        ...
        if os.path.exists(sanders):
            ...
            # and the algorithm predicts 0.0 by default (i.e., majority
            # class) the results are good.
            ...
            A, P, R, F = test(lambda review: en.positive(review, threshold=0.0),
                              reviews)
            # print(A, P, R, F)
...
class TestWordNet(unittest.TestCase):
    ...
    def test_synsets(self):
        ...
        # Assert TypeError when part-of-speech is not NOUN, VERB, ADJECTIVE
        # or ADVERB.
    ...
    def test_synset(self):
        ...
        self.assertTrue(
            "cougar" in v.synonyms)  # ["cougar", "puma", "catamount", ...]
        self.assertTrue(
            "feline" in v.gloss)  # "large American feline resembling a lion"
