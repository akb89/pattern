...
#   def cluster(self, method=HIERARCHICAL, k=1, iterations=1000)
    ...
    def resample(self, n=MINORITY):
        ...
        if n == MINORITY:
            n = min(list(p1.values()) or [0])
        if n == MAJORITY:
            n = max(list(p1.values()) or [0])
        a = []
        x = defaultdict(int)
        # 1) Undersampling.
        for d in self.documents:
        ...
        for t, i in list(p1.items()):
            for j in range(i, n):
                v = {}
                for f in shuffled(list(p3[t].keys())[:p2[t]]):
                    w = choice(p3[t][f]) 
...
def softmax(p):
    ...
    if p:
        v = list(p.values())
        m = max(v)
        e = [exp(x - m) for x in v] # prevent overflow
        s = sum(e)
        v = [x / s for x in e]
