...
>>> N['dog'].getSenses()
('dog' in {noun: dog, domestic dog, Canis familiaris}, 'dog' in {noun: frump,
dog}, 'dog' in {noun: dog}, 'dog' in {noun: cad, bounder, blackguard, dog,
hound, heel}, 'dog' in {noun: frank, frankfurter, hotdog, hot dog, dog,
wiener, wienerwurst, weenie}, 'dog' in {noun: pawl, detent, click, dog},
...
>>> dog.getPointers()[:5]
(hypernym -> {noun: canine, canid}, member meronym -> {noun: Canis,
genus Canis}, member meronym -> {noun: pack}, hyponym -> {noun: pooch,
doggie, doggy, barker, bow-wow}, hyponym -> {noun: cur, mongrel, mutt})
...
WNSEARCHDIR = environ.get('WNSEARCHDIR', os.path.join(WNHOME,
                                                      {'mac': "Database"}.get(
                                                          os.name, "dict")))
...
_FILE_OPEN_MODE = os.name in (
'dos', 'nt') and 'rb' or 'r'  # work around a Windows Python bug
...
def getSenses(self):
    ...
    >>> N['dog'].getSenses()
    ('dog' in {noun: dog, domestic dog, Canis familiaris}, 'dog' in {noun:
    frump, dog}, 'dog' in {noun: dog}, 'dog' in {noun: cad, bounder,
    blackguard, dog, hound, heel}, 'dog' in {noun: frank, frankfurter,
    hotdog, hot dog, dog, wiener, wienerwurst, weenie}, 'dog' in {noun: pawl,
    detent, click, dog}, 'dog' in {noun: andiron, firedog, dog, dog-iron})
    """
...
class Synset:
    ...
    def __init__(self, pos, offset, line):
        ...
        self.lexname = Lexname.lexnames and Lexname.lexnames[
            int(tokens[1])] or []
        (self._senseTuples, remainder) = _partition(tokens[4:], 2,
                                                    string.atoi(tokens[3], 16))
        (self._pointerTuples, remainder) = _partition(remainder[1:], 4,
                                                      int(remainder[0]))
        if pos == VERB:
            (vfTuples, remainder) = _partition(remainder[1:], 3,
                                               int(remainder[0]))

            def extractVerbFrames(index, vfTuples):
                return tuple([string.atoi(t[1]) for t in list(
    ...
    def getPointers(self, pointerType=None):
        ...
        (hypernym -> {noun: canine, canid}, member meronym -> {noun: Canis,
        genus Canis}, member meronym -> {noun: pack}, hyponym -> {noun:
        pooch, doggie, doggy, barker, bow-wow}, hyponym -> {noun: cur,
        mongrel, mutt})
        ...
        if pointerType == None:
            ...
            return list(
                filter(lambda pointer, type=pointerType: pointer.type == type,
                       self._pointers))
    ...
    def getPointerTargets(self, pointerType=None):
        ...
        [{noun: canine, canid}, {noun: Canis, genus Canis}, {noun: pack},
        {noun: pooch, doggie, doggy, barker, bow-wow}, {noun: cur, mongrel,
        mutt}]
    ...
    def __str__(self):
        ...
        """
        return "{" + self.pos + ": " + string.joinfields(
                [sense.form for sense in self.getSenses()], ", ") + "}"
    ...
    def __getitem__(self, idx):
        ...
        if isinstance(idx, StringType):
            idx = _index(idx, [sense.form for sense in senses]) or \
                  _index(idx, [sense.form for sense in senses],
                         _equalsIgnoreCase)
        return senses[idx]
...
class Sense:
    """A specific meaning of a specific word -- the intersection of a Word
    and a Synset.
    ...
    def getPointers(self, pointerType=None):
        ...
        def pointsFromThisSense(pointer, selfIndex=senseIndex):
            return pointer.sourceIndex == 0 or pointer.sourceIndex - 1 == \
											   selfIndex

        return list(
            filter(pointsFromThisSense, self.synset.getPointers(pointerType)))
    ...
    def __cmp__(self, other):
        def senseIndex(sense, synset=self.synset):
            return _index(sense, synset.getSenses(),
                          testfn=lambda a, b: a.form == b.form)

        return _compareInstances(self, other, ('synset',)) or cmp(
            senseIndex(self), senseIndex(other))
...
class Pointer:
    ...
    def __cmp__(self, other):
        ...
        def pointerIndex(sense, synset=synset):
            return _index(sense, synset.getPointers(),
                          testfn=lambda a, b: not _compareInstances(a, b, (
                          'type', 'sourceIndex', 'targetIndex')))
...
class Dictionary:
    ...
    def __init__(self, pos, filenameroot):
        ...
        self.dataFile = [(open(f, _FILE_OPEN_MODE), os.stat(f)[6]) for f in
                         _dataFilePathname(filenameroot)]  # Tom De Smedt, 2011
    ...
    def getWord(self, form, line=None):
        ...
        if word:
            ...
            raise KeyError(
                "%s is not in the %s database" % (repr(form), repr(pos)))
    ...
    def __contains__(self, form):
        return form.encode("utf-8",
                           "ignore") in self.indexFile  # Tom De Smedt, 2013
...
def _dataFilePathname(filenameroot):
    ...
    import glob
    return sorted(
        glob.glob(os.path.join(WNSEARCHDIR, "data." + filenameroot + "*")))
...
def _lineAt(files, offset):  # Tom De Smedt, 2011
    for file, size in files:  # Seek across multiple files (i.e., data.noun1
		# + data.noun2).
        if offset < size:  # Purpose: Google App Engine requires filesize <
			# 10MB.
            break
        offset -= size
    file.seek(offset)
