...
####################################################################################################
# Template for pattern.xx, bundling natural language processing tools for
# language XXXXX.
# The module bundles a shallow parser (part-of-speech tagger, chunker,
# lemmatizer)
# with functions for word inflection (singularization, pluralization,
# conjugation)
# and sentiment analysis.

# Base classes for the parser, verb table and sentiment lexicon are inherited
#  from pattern.text.
# The parser can be subclassed with a custom tokenizer (finds sentence
# boundaries)
...
from pattern.text import (
    Lexicon, Model, Morphology, Context, Parser as _Parser, ngrams, pprint,
    commandline,
    PUNCTUATION
)
...
# Pattern uses the Penn Treebank II tagset (
# http://www.clips.ua.ac.be/pages/penn-treebank-tagset).
# The lexicon for pattern.xx may be using a different tagset (e.g., PAROLE,
# WOTAN).
...
# Different languages have different contractions (e.g., English "I've" or
# French "j'ai")
# and abbreviations. The following functions define contractions and
# abbreviations
...
# and rules for unknown words. See pattern.text.Morphology and
# pattern.text.Context
# for further details. A tutorial on how to acquire data for the lexicon is
# here:
# http://www.clips.ua.ac.be/pages/using-wiktionary-to-build-an-italian-part
# -of-speech-tagger
...
def tag(s, tokenize=True, encoding="utf-8", **kwargs):
    ...
    """
    tags = []
    for sentence in parse(s, tokenize, True, False, False, False, encoding,
                          **kwargs).split():
...
def subjectivity(s, **kwargs):
    """ Returns the sentence subjectivity (objective/subjective) between 0.0
    and 1.0.
    """
