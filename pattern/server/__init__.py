...
# On Linux + Apache mod_wsgi, the user that executes the Python script is
# "www-data".
# If the app folder was created by "root", "www-data" will not have write
# permission,
# and consequently cannot write to an SQLite database (e.g., App.rate) in the
#  folder,
# or create SQLite -journal files. 

# The solution is for "www-data" to chown() the folder, and any database
# files in it.
...
def encode_entities(string):
    """ Encodes HTML entities in the given string ("<" => "&lt;").
        ...
        we need to pass "&lt;em&gt;hello&lt;/em&gt;" (otherwise "hello" in
        italic is displayed).
    """
...
def encode_url(string):
    return urllib.parse.quote_plus(
            string.encode("utf-8"))  # "black/white" => "black%2Fwhite".
...
#### INTROSPECTION
# ##############################################################################
# URL paths are routed to handler functions, whose arguments represent URL
...
def define(f):
    ...
    def undecorate(f):  # "__closure__" in Py3.
        while getattr(f, "func_closure", None):
            ...
            f = [v for v in f if isinstance(v, collections.Callable)]
...
class Database(object):
    ...
    def connect(self):
        ...
        if self._type == MYSQL:
            ...
            self._connection.cursor().execute(
                    "create database if not exists `%s`" % self._name)
    ...
    @property
    def batch(self):
        return Database._batch.setdefault(self._name,
                                          DatabaseTransaction(self._name,
                                                              **self.__dict__))
...
class DatabaseTransaction(Database):
    def __init__(self, name, **kwargs):
        """ Database.batch.execute() stores given the SQL query in RAM
        memory, across threads.
            Database.batch.commit() commits all buffered queries.
            This can be combined with @app.task() to periodically write
...
def verify_password(s1, s2):
    """ Returns True if the given strings are identical, after hashing the
    first.
    """
...
# 2) Download MySQL .dmg: http://dev.mysql.com/downloads/mysql/ (for 64-bit
# Python, 64-bit MySQL).
...
#### RATE LIMITING
# ##############################################################################
# With @app.route(path, limit=True), the decorated URL path handler function
# calls RateLimit().
# For performance, rate limiting uses a RAM cache of api keys + the time of
# the last request.
...
class RateLimit(Database):
    def __init__(self, name="rate.db", **kwargs):
        """ A database for rate limiting API requests.
            ...
            It grants each key a rate (number of requests / time) for a URL
            path.
            It keeps track of the number of requests in local memory (i.e.,
            RAM).
            If RateLimit()() is called with the optional limit and time
            arguments,
    ...
    def set(self, key, path="/", limit=100, time=HOUR):
        """ Sets the rate for the given key and path,
            where limit is the maximum number of requests in the given time (
            e.g., 100/hour).
        """
        # Map time as str to float.
        time = {"second": SECOND, "minute": MINUTE, "hour": HOUR,
                "day"   : DAY}.get(time, time)
        # Update database.
        p = "/" + path.strip("/")
    ...
    def __contains__(self, key, path="%"):
        ...
        return self.execute(q, (key, path), first=True,
                            commit=False) is not None

    def __call__(self, key, path="/", limit=None, time=None, ip=None,
                 reset=100000):
        """ Increases the (cached) request count by 1 for the given key and
        path.
        ...
        with self.lock:
            ...
            if r is None and ip is not None and limit is not None and time is \
                    not None:
...
#### ROUTER
# ##############################################################################
# The @app.route(path) decorator registers each URL path handler in
# Application.router.
    ...
    def __call__(self, path, **data):
        ...
        for i in range(n + 1):
            ...
            if p0 in self:
                ...
                return handler(*p1, **dict(
                        (k, v) for k, v in list(data.items()) if k in kwargs))
        # No handler.
        raise RouteError
...
class HTTPError(Exception):
    def __init__(self, status="", message="", traceback=""):
        """ A HTTP error raised in an @app.route() handler + passed to
        @app.error().
        """
...
def _HTTPErrorSubclass(status):
    return type("HTTP%sError" % status.split(" ")[0], (HTTPError,),
                {'__init__': \
                     lambda
                         self,
                         message="",
                         traceback="": HTTPError.__init__(
                             self,
                             status,
                             message,
                             traceback)})
...
# ------------------------------------------------------------------
# With a multi-threaded server, each thread requires its own local data (
...
def threadsafe(function):
    """ The @threadsafe decorator ensures that no two threads execute the
    function simultaneously.
    """
    # In some cases, global data must be available across all threads (e.g.,
    # rate limits).
    # Atomic operations like dict.get() or list.append() (= single execution
    # step) are thread-safe,
...
class Application(object):
    def __init__(self, name=None, path=SCRIPT, static="./static",
                 rate="rate.db", owner=None):
        """ A web app served by a WSGI-server that starts with App.run().
            By default, the app is served from the folder of the script that
            imports pattern.server.
    ...
    def route(self, path, limit=False, time=None,
              key=lambda data: data.get("key"), reset=100000):
        """ The @app.route(path) decorator defines the handler function for
        the given path.
            The function can take arguments (path) and keyword arguments (
            query data), e.g.,
            if no handler exists for URL "/api/1/en", but a handler exists
            for URL "/api/1",
            this handler will be called with 1 argument: "en".
            It returns a string, a generator or a dictionary (which is parsed
            to a JSON-string).
        """
        _a = (key, limit, time,
              reset)  # Avoid ambiguity with key=lambda inside define().
        ...
        def decorator(handler):
            def ratelimited(handler):
                ...
                # RateLimit()(key, path, limit, time) before calling the
                # handler.
                # By default, a query parameter "key" is expected.
                # If the key is known, apply rate limiting (429 Too Many
                # Requests).
            ...
            if limit is True or (
                                limit is not False and limit is not None and
                            time is
                    not None):
    ...
    def error(self, code="*"):
        """ The @app.error(code) decorator defines the handler function for
        the given HTTP error.
        ...
        def decorator(handler):
            ...
            def wrapper(status="", message="", traceback="", version=""):
                # Avoid CherryPy bug "ValueError: status message was not
                # supplied":
    ...
    def view(self, template, cached=True):
        """ The @app.view(template) decorator defines a template to format
        the handler function.
            The function returns a dict of keyword arguments for
            Template.render().
        """
        ...
        def decorator(handler):
            def wrapper(*args, **kwargs):
                if not hasattr(template,
                               "render"):  # bottle.py templates have render(
                    # ) too.
    ...
    class Thread(localdict):
        """ The @app.thread(event) decorator can be used to initialize
        thread-safe data.
            Get data (e.g., a database connection) with app.thread.[name] or
            g.[name].
        """
    ...
    def bind(self, name="db"):
        """ The @app.bind(name) decorator binds the given function to a
        keyword argument
            that can be used with @app.route() handlers.
            The return value is stored thread-safe in app.thread.[name] & g.[
            name].
            The return value is available in handlers as a keyword argument [
            name].
        """
    ...
    def __getattr__(self, k):
        """ Yields the value of the bound function with the given name (e.g.,
        app.db).
        """
    ...
    @property
    def cached(self):
        """ The @app.cached decorator caches the return value of the given
        handler.
        ...
        def decorator(handler):
            def wrapper(*args, **kwargs):
                ...
                k = sorted((k, v) for k, v in list(kwargs.items()) if
                           not isinstance(v, Database))
    ...
    def task(self, interval=MINUTE):
        """ The @app.task(interval) decorator will call the given function
        repeatedly (in a thread).
            For example, this can be used to commit a Database.batch
            periodically,
            instead of executing and committing to a Database during each
            request.
        """
        ...
        def decorator(handler):
            ...
            def wrapper():
                ...
                # pass it as a keyword argument named "db".
                return handler(
                        **dict((k, v) for k, v in list(g.items()) if
                               k in kwargs))
    ...
    def run(self, host=LOCALHOST, port=8080, threads=30, queue=20, timeout=10,
            sessions=False, embedded=False, xhr="*", ssl=None, debug=True):
        """ Starts the server.
            Static content (e.g., "g/img.jpg") is served from the App.static
            subfolder (e.g., "static/g").
            ...
            With xhr="*", the server will respond to cross-origin
            XMLHttpRequests.
            With ssl=(key, certificate), runs under https:// (see
        ...
        if ssl:
            cp.config.update({
                ...
                "server.ssl_private_key": ssl[0] if os.path.exists(
                        ssl[0]) else openable(ssl[0]),
                "server.ssl_certificate": ssl[1] if os.path.exists(
                        ssl[1]) else openable(ssl[1])
            })
        ...
        self._app = cp.tree.mount(self, "/",
                                  config={"/": {
                                      "tools.staticdir.on"    : self.static
                                                                is not None,
                                      "tools.staticdir.dir"   : self.static,
                                      "tools.sessions.on"     : bool(sessions),
                                      "tools.sessions.timeout": 60 if
                                      sessions is True else int(
                                              sessions)
                                  }})
...
# The certificate() function yields a free, self-signed certificate (valid
# for 365 days).
# Visitors will get a browser warning that the certificate is not signed by a
#  trusted third party.

def certificate(domain=LOCALHOST, country=None, state=None, city=None,
                company=None, contact=None, signed=True, **kwargs):
    """ Returns a (private key, certificate)-tuple for a secure SSL-encrypted
    https server.
...
# k, x = certificate(country="BE", state="Antwerp", company="CLiPS",
...
# 
# 1) Create a DNS-record for APP, which maps the url to your server's
# IP-address.
#
...
#    >
#    > app = application = App() # mod_wsgi app must be available as
# "application"!
#    >
...
#### TEMPLATE
# ##############################################################################
# A template is a HTML-file with placeholders, which can be variable names or
#  Python source code.
# Based on: http://davidbau.com/archives/2011/09/09
# /python_templating_with_stringfunction.html
...
class Template(object):
    ...
    def __init__(self, path, root=None, cached=True):
        ...
        given string or path.
            Placeholders that start with $ are replaced with keyword
            arguments in Template.render().
        ...
        if cached is False or b is False:
            a = "".join(static(p, mimetype="text/html")) if os.path.exists(
                    p) else path
            a = self._compile(a)
    ...
    def _compile(self, string):
        ...
        for m in _MARKUP.finditer(string):
            ...
            # <% if x in y: %> ... <% end if %>
                ...
                a.append(("<if>",
                          (m.group(2), self._compile(m.group(3).lstrip("\n"))),
                          w))
            # <% for x in y: %> ... <% end for %>
            elif s.startswith("<%") and m.group(4):
                a.append(("<for>", (
                    m.group(4), m.group(5),
                    self._compile(m.group(6).lstrip("\n"))),
                          w))
            # <%= var + 1 %>
            elif s.startswith("<%=") and s.endswith("%>"):
                a.append(("<eval>",
                          compile("\n" * n + self._escape(s[3:-2]), "<string>",
                                  "eval"), w))
            # <% print(var) %>
            elif s.startswith("<%") and s.endswith("%>"):
                a.append(("<exec>",
                          compile("\n" * n + self._escape(s[2:-2]), "<string>",
                                  "exec"), w))
            else:
                raise SyntaxError(
                        "template has no end tag for '%s' (line %s)" % (
                        s, n + 1))
            i = m.end(1)
    ...
    def _render(self, compiled, *args, **kwargs):
        ...
        for cmd, v, w in compiled:
            ...
            if cmd is None:
                ...
                yield "".join(
                        ["".join(self._render(v[2], k, self._dict(v[0], i))) for
                         i in eval(v[1], k)])
            elif cmd == "<eval>":
    ...
    def render(self, *args, **kwargs):
        """ Returns the rendered template as a string.
            ...
            For example, source code in Template.render(re=re) has access to
            the regex library.
...
# @app.bind("db")
# def db():
#    return Database("log.db", schema="create table if not exists `log` (q
# text);")
#
#
