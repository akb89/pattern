...
class Counter(dict):
    ...
    def update(self, iterable=None, **kwargs):
        """ Updates counter with the tallies from the given iterable,
...
class freq(Counter):
    ...
    def keys(self):
        return [k for k, v in list(self.items())]
...
ACCURACY, PRECISION, RECALL, F1_SCORE = "accuracy", "precision", "recall", \
                                        "F1-score"

MACRO = "macro"


def confusion_matrix(classify=lambda document: False,
                     documents=[(None, False)]):
    """ Returns the performance of a binary classification task (i.e.,
    predicts True or False)
        ...
        where True means a document that should be identified as True by
        classify().
    """
    TN = TP = FN = FP = 0
...
def test(classify=lambda document: False, documents=[], average=None):
    """ Returns an (accuracy, precision, recall, F1-score)-tuple.
        With average=None, precision & recall are computed for the positive
        class (True).
        With average=MACRO, precision & recall for positive and negative
        class are macro-averaged.
    """
...
def accuracy(classify=lambda document: False, documents=[], average=None):
    """ Returns the percentage of correct classifications (true positives +
    true negatives).
    """
...
def auc(curve=[]):
    """ Returns the area under the curve for the given list of (x, y)-points.
        ...
        the return value is the probability (0.0-1.0) that a classifier will
        rank
        a random positive document (True) higher than a random negative one (
        False).
    """
    curve = sorted(curve)
    # Trapzoidal rule: area = (a + b) * h / 2, where a=y0, b=y1 and h=x1-x0.
    return sum(0.5 * (x1 - x0) * (y1 + y0) for (x0, y0), (x1, y1) in
               sorted(zip(curve, curve[1:])))
...
def fleiss_kappa(m):
    """ Returns the reliability of agreement as a number between -1.0 and +1.0,
        ...
        For example, say 5 people are asked to vote "cat" and "dog" as "good"
        or "bad":
         m = [# + -        
               [3,2], # cat
               [5,0]] # dog
    ...
    if n == 1:
        return 1.0
    assert all(sum(row) == n for row in
               m[1:]), "numer of votes for each task differs"
    ...
    P = [(sum(m[i][j] ** 2 for j in range(k)) - n) / float(n * (n - 1)) for i in
         range(N)]
...
def levenshtein(string1, string2):
    ...
    for i in range(1, m + 1):
        ...
        for j in range(1, n + 1):
            insert, delete, replace = previous[j] + 1, current[j - 1] + 1, \
                                      previous[j - 1]
...
def levenshtein_similarity(string1, string2):
    """ Returns the similarity of string1 and string2 as a number between 0.0
    and 1.0.
    """
    return 1 - levenshtein(string1, string2) / float(
        max(len(string1), len(string2), 1.0))


def dice_coefficient(string1, string2):
    """ Returns the similarity between string1 and string1 as a number
    between 0.0 and 1.0,
        based on the number of shared bigrams, e.g., "night" and "nacht" have
        one common bigram "ht".
    """
...
def flesch_reading_ease(string):
    ...
    # which uses simpler syllable counting (count_syllables() is the weak
    # point here).
...
# For example, on the Corpus of Plagiarised Short Answers (Clough &
# Stevenson, 2009),
...
def ngrams(string, n=3, punctuation=PUNCTUATION, **kwargs):
    """ Returns a list of n-grams (tuples of n successive words) from the
    given string.
...
def intertextuality(texts=[], n=5, weight=lambda ngram: 1.0, **kwargs):
    """ Returns a dictionary of (i, j) => float.
        ...
        the corresponding float is the percentage of text i that is also in
        text j.
        Overlap is measured by matching n-grams (by default, 5 successive
        words).
        An optional weight function can be used to supply the weight of each
        n-gram.
    """
...
def type_token_ratio(string, n=100, punctuation=PUNCTUATION):
    """ Returns the percentage of unique words in the given string as a
    number between 0.0-1.0,
        as opposed to the total number of words (= lexical diversity,
        vocabulary richness).
    """
...
def suffixes(inflections=[], n=3, top=10, reverse=True):
    """ For a given list of (base, inflection)-tuples,
        returns a list of (count, inflected suffix, [(base suffix,
        frequency), ... ]):
        suffixes([("beau", "beaux"), ("jeune", "jeunes"), ("hautain",
        "hautaines")], n=3) =>
...
def cooccurrence(iterable, window=(-1, -1), term1=lambda x: True,
                 term2=lambda x: True, normalize=lambda x: x, matrix=None,
                 update=None):
    """ Returns the co-occurence matrix of terms in the given iterable,
    string, file or file list,
        ...
        The normalize() function can be used to remove punctuation, lowercase
        words, etc.
        Optionally, a user-defined matrix to update can be given.
        Optionally, a user-defined update(matrix, term1, term2, index2)
    ...
    if isinstance(iterable, (list, tuple)) and all(
            hasattr(f, "read") for f in iterable):
    ...
    m = matrix
    # Search terms may fall outside the co-occurrence window, e.g., window=(
    # -3,-2).
        ...
        if len(q) > n:
            ...
            if not isinstance(x1, Sentinel):
                ...
                if term1(x1):
                    ...
                    for j, x2 in enumerate(list(q).__getslice__(i + window[0],
                                                                i + window[
                                                                    1] + 1)):
...
## {("cat", "NN"): {("black", "JJ"): 1}}
# s = [("The","DT"), ("black","JJ"), ("cat","NN"), ("sat","VB"), ("on","IN"),
#  ("the","DT"), ("mat","NN")]
...
def lerp(a, b, t):
    """ Returns the linear interpolation between a and b at time t between
    0.0-1.0.
...
def smoothrange(a=None, b=None, n=10):
    """ Returns an iterator of approximately n values v1, v2, ... vn,
        so that v1 <= a, and vn >= b, and all values are multiples of 1, 2,
        5 and 10.
        For example: list(smoothrange(1, 123)) => [0, 20, 40, 60, 80, 100,
        120, 140],
    """
...
def median(iterable, sort=True):
    """ Returns the value that separates the lower half from the higher half
...
def simple_moving_average(iterable, k=10):
    """ Returns an iterator over the simple moving average of the given list
    of values.
    """
    a = iterable if isinstance(iterable, list) else list(iterable)
...
def moment(iterable, n=2, sample=False):
    """ Returns the n-th central moment of the given list of values
        (2nd central moment = variance, 3rd and 4th are used to define
...
def skewness(iterable, sample=False):
    """ Returns the degree of asymmetry of the given list of values:
        ...
        = 0.0 => evenly distributed on both sides of the mean (= normal
        distribution).
    """
...
def kurtosis(iterable, sample=False):
    """ Returns the degree of peakedness of the given list of values:
        > 0.0 => sharper peak around mean(list) = more infrequent, extreme
        values,
...
def quantile(iterable, p=0.5, sort=True, a=1, b=-1, c=0, d=1):
    """ Returns the value from the sorted list at point p (0.0-1.0).
        If p falls between two items in the list, the return value is
        interpolated.
...
def boxplot(iterable, **kwargs):
    """ Returns a tuple (min(list), Q1, Q2, Q3, max(list)) for the given list
    of values.
...
def fisher_exact_test(a, b, c, d, **kwargs):
    ...
    # Based on: http://www.koders.com/java
    # /fid868948AD5196B75C4C39FEA15A0D6EAF34920B55.aspx?s=252
    s = [cutoff] + \
        [p(a + i, b - i, c - i, d + i) for i in
         range(1, min(int(b), int(c)) + 1)] + \
        [p(a - i, b + i, c + i, d - i) for i in
         range(1, min(int(a), int(d)) + 1)]
...
def pearson_chi_squared_test(observed=[], expected=[], df=None, tail=UPPER):
    ...
    absolute frequencies).
        ...
        p < 0.05: significant
        p < 0.01: very significant
        This means that if p < 5%, the data is unevenly distributed (e.g.,
        biased).
    ...
    """
    # The p-value (upper tail area) is obtained from the incomplete gamma
    # integral:
    # p(x2 | v) = gammai(v/2, x/2) with v degrees of freedom.
    # See: Cephes, https://github.com/scipy/scipy/blob/master/scipy/special
    # /cephes/chdtr.c
    o = list(observed)
...
def kolmogorov_smirnov_two_sample_test(a1, a2=NORMAL, n=1000):
    """ Returns the likelihood that two independent samples are drawn from
...
def pdf(x, mean=0.0, stdev=1.0):
    """ Returns the probability density function at x:
        the likelihood of x in a distribution with given mean and standard
        deviation.
    """
...
def kolmogorov(x):
    ...
    x = -2.0 * x * x
    k = 0
    for i in reversed(list(range(1, 27 + 1, 2))):  # 27 25 23 ... 1
        k = (1 - k) * exp(x * i)
    return 2.0 * k
